...
ffmpeg -hide_banner -i "$out" -i "$source_y4m" -lavfi '[0:v]setpts=PTS-STARTPTS[d];[1:v]setpts=PTS-STARTPTS[r];[d][r]psnr' -frames:v "$frames" -f null - 2> "$out.psnr.log"
ffmpeg -hide_banner -i "$out" -i "$source_y4m" -lavfi '[0:v]setpts=PTS-STARTPTS[d];[1:v]setpts=PTS-STARTPTS[r];[d][r]ssim' -frames:v "$frames" -f null - 2> "$out.ssim.log"
...
ffmpeg -hide_banner -loglevel error -i "$out" -map 0:v:0 -frames:v "$frames" -f framehash -hash sha256 "$out.framehash"
